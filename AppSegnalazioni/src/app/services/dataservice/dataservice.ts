...
import { AppReport, ImageDTO } from '../../models/app-report';
...
export class DataService {
    ...
    return fetch('./assets/report.geojson')
      ...
      .then((geojson: any) => {
        return geojson.features.map((f: any, index: number) => ({
          ...
          categoryNames: f.properties.category ? [f.properties.category] : ['Others'],
          images: f.properties.image ? [{ base64: f.properties.image }] : [] as ImageDTO[],
  ...
  getUserReports(userId: number): Promise<AppReport[]> {
    return fetch(`http://localhost:5077/api/user/${userId}/reports`)
      .then(resp => resp.json())
